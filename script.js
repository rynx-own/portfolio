...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function triggerHeroAnimations() {
        const heroElements = document.querySelectorAll('.hero-title-new, .hero-subtitle-new, .hero-quote-new, .badge-grid, .scroll-hint-new, .profile-wrapper');
        heroElements.forEach(el => {
            el.style.animation = el.style.animation; // Restart animation
        });
    }
    ...
    if (typedText) {
        new Typed('#typed-text', {
            strings: [
                'Creative Developer yang suka explore hal hal baru. <strong>Coding</strong> by day, <strong>vibing to music</strong> by night. Let\'s create something <span class="text-purple">aesthetic</span> together!'
            ],
            typeSpeed: 60,
            backSpeed: 0,
            showCursor: true,
    ...
    window.addEventListener('scroll', function() {
        if (window.scrollY > 50) {
            navbarNew?.classList.add('scrolled');
        } else {
            navbarNew?.classList.remove('scrolled');
        }
    });
